...
function HtmlWebpackIncludeAssetsPlugin(options) {
  ...
  if (options.resolvePaths !== undefined) {
    assert(
      isBoolean(options.resolvePaths),
      'HtmlWebpackIncludeAssetsPlugin options should specify a resolvePaths that is a boolean'
    );
  }
  ...
  assert(
    isArray(assets),
    'HtmlWebpackIncludeAssetsPlugin options must have an assets key with an array or string value'
  );
  var jsExtensions;
  if (options.jsExtensions !== undefined) {
    if (isString(options.jsExtensions)) {
      ...
      jsExtensions = options.jsExtensions;
      assert(
        isArray(jsExtensions),
        'HtmlWebpackIncludeAssetsPlugin options jsExtensions key should be a string or array of strings (' +
          jsExtensions +
          ')'
      );
      ...
      for (var j = 0; j < jsExtensionCount; j++) {
        ...
        assert(
          isString(jsExtension),
          'HtmlWebpackIncludeAssetsPlugin options jsExtensions key array should not contain non-strings (' +
            jsExtension +
            ')'
        );
      }
  ...
  if (options.cssExtensions !== undefined) {
    if (isString(options.cssExtensions)) {
      ...
      cssExtensions = options.cssExtensions;
      assert(
        isArray(cssExtensions),
        'HtmlWebpackIncludeAssetsPlugin options cssExtensions key should be a string or array of strings (' +
          cssExtensions +
          ')'
      );
      ...
      for (var c = 0; c < cssExtensionCount; c++) {
        ...
        assert(
          isString(cssExtension),
          'HtmlWebpackIncludeAssetsPlugin options cssExtensions key array should not contain non-strings (' +
            cssExtension +
            ')'
        );
      }
  ...
  for (var i = 0; i < assetCount; i++) {
    ...
    if (isString(asset)) {
      assert(
        hasExtensions(asset, jsExtensions) || hasExtensions(asset, cssExtensions),
        'HtmlWebpackIncludeAssetsPlugin options assets key array should only contain strings ending with the js or css extensions (' +
          asset +
          ')'
      );
      ...
      assert(
        isString(asset.path),
        'HtmlWebpackIncludeAssetsPlugin options assets key array objects path property must be a string (' +
          asset.path +
          ')'
      );
      if (asset.glob !== undefined) {
        assert(
          isString(asset.glob),
          'HtmlWebpackIncludeAssetsPlugin options assets key array objects glob property should be a string (' +
            asset.glob +
            ')'
        );
      }
      if (asset.globPath !== undefined) {
        assert(
          isString(asset.globPath),
          'HtmlWebpackIncludeAssetsPlugin options assets key array objects globPath property should be a string (' +
            asset.globPath +
            ')'
        );
      }
      if (asset.assetPath !== undefined) {
        assert(
          isString(asset.assetPath),
          'HtmlWebpackIncludeAssetsPlugin options assets key array objects assetPath property should be a string (' +
            asset.assetPath +
            ')'
        );
      }
      if (asset.type !== undefined) {
        assert(
          isOneOf(asset.type, ['js', 'css']),
          'HtmlWebpackIncludeAssetsPlugin options assets key array objects type property should be a string set to either `js` or `css` (' +
            asset.type +
            ')'
        );
      } else {
        if (asset.glob !== undefined) {
          assert(
            hasExtensions(asset.glob, jsExtensions) || hasExtensions(asset.glob, cssExtensions),
            'HtmlWebpackIncludeAssetsPlugin options assets key array objects glob property should only contain strings ending with the js or css extensions if the type property is not set (' +
              asset +
              ')'
          );
        } else {
          assert(
            hasExtensions(asset.path, jsExtensions) || hasExtensions(asset.path, cssExtensions),
            'HtmlWebpackIncludeAssetsPlugin options assets key array objects path property should only contain strings ending with the js or css extensions if the type property is not set (' +
              asset +
              ')'
          );
        }
      ...
      if (asset.attributes !== undefined) {
        assert(
          isObject(asset.attributes),
          'HtmlWebpackIncludeAssetsPlugin options assets key array objects attributes property should be an object'
        );
        forOwn(asset.attributes, function(value) {
          assert(
            isString(value) || isBoolean(value),
            'HtmlWebpackIncludeAssetsPlugin options assets key array objects attributes property should be an object with string or boolean values'
          );
        });
      }
    } else {
      assert(
        false,
        'HtmlWebpackIncludeAssetsPlugin options assets key array must contain only strings and objects (' +
          asset +
          ')'
      );
    }*/
  ...
  assert(
    isBoolean(options.append),
    'HtmlWebpackIncludeAssetsPlugin options must have an append key with a boolean value'
  );
  var publicPath;
  if (options.publicPath !== undefined) {
    assert(
      isBoolean(options.publicPath) || isString(options.publicPath),
  ...
  if (options.hash !== undefined) {
    assert(
      isBoolean(options.hash),
      'HtmlWebpackIncludeAssetsPlugin options should specify a hash key with a boolean value'
    );
    hash = options.hash;
  } else {
  ...
  if (files !== undefined) {
    assert(
      isArray(files),
      'HtmlWebpackIncludeAssetsPlugin options should specify a files key with an array or string value'
    );
    ...
    for (var f = 0; f < fileCount; f++) {
      ...
      assert(
        isString(file),
        'HtmlWebpackIncludeAssetsPlugin options files key array must contain only strings (' +
          file +
          ')'
      );
    }
  ...
  function onCompilation(compilation) {
    ...
    var getAssetPath = function(includeAssetPath) {
      ...
      var includeAssetPrefix =
        publicPath === true ? defaultPublicPath : isString(publicPath) ? publicPath : '';
      ...
      for (var i = 0; i < includeCount; i++) {
        ...
        if (isObject(includeAsset)) {
          ...
          if (includeAsset.glob === undefined) {
            ...
            var cwd =
              includeAsset.globPath !== undefined
                ? includeAsset.globPath
                : path.join(compiler.options.output.path, includeAsset.path);
            var globOptions = { cwd: cwd };
            // assets will be an array of strings with all matching asset file names
            includeAssetPaths = glob.sync(includeAsset.glob, globOptions).map(function(globAsset) {
        ...
        for (var a = 0; a < includeAssetCount; a++) {
          ...
          if (
            (includeAssetType && includeAssetType === 'js') ||
            hasExtensions(includeAssetString, jsExtensions)
          ) {
            if (
              assets.js.indexOf(includeAssetString) === -1 &&
              jsAssets.indexOf(includeAssetString) === -1
            ) {
              jsAssets.push(includeAssetString);
            }
          } else if (
            (includeAssetType && includeAssetType === 'css') ||
            hasExtensions(includeAssetString, cssExtensions)
          ) {
            if (
              assets.css.indexOf(includeAssetString) === -1 &&
              cssAssets.indexOf(includeAssetString) === -1
            ) {
    ...
    function onAlterAssetTag(htmlPluginData, callback) {
      ...
      for (var i = 0; i < tagCount; i++) {
        tag = tags[i];
        assetAttributes = findAttributesForAsset(
          includeAssets,
          tag.attributes && (tag.attributes.href || tag.attributes.src)
        );
        if (assetAttributes) {
